    ...
    def post(self, request, *args, **kwargs):
        ...
        if order_status:
            ...
            if order_status == Order.Status.CONFIRMED:
                Cart.objects.filter(user_id=user_id, is_archived=False).update(
                    is_archived=True
                )

                Invoice.objects.filter(order_id=order_id).update(
...
def get_event_from_request(request):
    ...
    Log.objects.create(
        event_type=event_type, details=details, error_message=error_message
    )
    return event
